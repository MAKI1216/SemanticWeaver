...
- **玩家推理逻辑审核** ✅ 已完成（2026-06-19，团队协作）
  - 策划师季元策+文案师温思语独立审核 → 主理人汇编 `docs/玩家推理逻辑审核文档.md`
  - 核心问题：4/7处嗅觉重构不合理、Meta卡缺教学引导、4个关键词缺metadata、"擦窗杯"命名错误
  - 难度曲线：2→3→3→2，修复假性难度后1.5→2.5→2.5→2
  - 共25个问题（8高+9中+8低），16个关键问题已完成整改（2026-06-19 凌晨）
- **审核问题整改** ✅ 已完成（2026-06-19，季元策执行）
  - H1 4处嗅觉→视觉（T2-S02杯/T2-S04项链/T3-S04金属→视觉；T1-S01保留嗅觉+对话铺垫）
  - H3 "擦窗杯"→"搪瓷杯底发射器"（对话+配方+metadata全链路修复）
  - H4/H5/H8 补充缺失metadata、hidden_keyword_sources、unlocks_hidden_layer
  - H6/L7 Trial 1矛盾新增 reveal_text 旁白叙事（game.js onContradictionFlagged 扩展）
  - H7 倒计时179→60秒统一（T4对话+outro全部修正）
  - M1/M3/M7/M8 听觉/矛盾/伤疤/脑波异常对话优化
  - L1-L5 文案细节修复（铅板→铁皮、英文→中文、hidden_layer去重等）
  - 修改涉及 game-data.js (20+处编辑)、tutorial.js (T7)、game.js (reveal_text)
